  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-emerald-100 bg-white/70 backdrop-blur-md transition-all duration-300 dark:border-emerald-950/60 dark:bg-slate-900/80">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        {/* Logo - Geometric styled */}
        <div id="app-logo" className="flex items-center gap-3 cursor-pointer select-none" onClick={() => setTab('dashboard')}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-xs font-bold transition-all duration-300 rounded-xl ${
                  isActive
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden sm:flex items-center gap-2 px-3.5 py-1.5 bg-white dark:bg-slate-900 rounded-full border border-emerald-100 dark:border-emerald-950/60 shadow-sm">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-emerald-100 bg-white text-slate-500 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-350 dark:hover:bg-slate-800"
          >
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 md:hidden dark:border-slate-800 dark:bg-slate-900 dark:text-slate-350"
          >
      ...
      {/* Mobile Menu Panel */}
      {mobileMenuOpen && (
        <div id="mobile-menu" className="border-t border-slate-100 bg-white px-4 py-2 space-y-1 md:hidden dark:border-slate-800 dark:bg-slate-900 shadow-xl">
          {navItems.map((item) => {
            const isActive = currentTab === item.id;
            return (
              <button
                ...
                className={`flex w-full items-center px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  isActive
          ...
          <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-3 pb-1.5 px-4 text-xs">
